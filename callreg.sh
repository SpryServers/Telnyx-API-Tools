...
while [[ "$#" -gt 0 ]]; do
    case $1 in
      ...
      ;;
      -gn | --get-numbers ) INPUT=1 && cat verifiedcalls/numbers.config
      ;;
      -gr | --get-reasons ) INPUT=1 && cat verifiedcalls/callreasons.config
      ;;
      -h | --help | \? ) INPUT=1 \
      echo "Usage:"
      echo ""
...
if [[ "$REGISTERCALL" -eq 1 ]]; then
    if [ -z $APIKEY ]; then
        echo "No API key specified in config. Please update api.config file."
        exit 1
    fi
    call_register
fi
